...
def slice_flow_specimen(
    ...
    *layer_height* and *extrusion_width*.
    ...
    if binary_gcode:
        cli_extra.append("--binary-gcode")
    # Vase-mode tools default to a 5 mm brim with 0.5 mm separation for
    ...
    eff_brim_sep = brim_separation if brim_separation is not None else 0.5
...
infill.  PrusaSlicer enables bridging detection by default, so no special
overrides are needed.
"""


def slice_bridge_specimen(
    stl_path: str,
    output_gcode_path: str,
    layer_height: float = 0.2,
    extrusion_width: float = 0.45,
    ...
    Uses standard slicing with PrusaSlicer's default bridging detection
    enabled.  No special forced settings are needed.

    When *config_ini* is ``None``, :data:`BRIDGE_SLICER_ARGS` are
    applied together with the explicit *layer_height* and *extrusion_width*.
...
Same structural settings as the temperature tower — 2 perimeters and 15%
infill.  Support material is always forced off so the user can evaluate
overhang quality without supports.
"""


def slice_overhang_specimen(
    stl_path: str,
    output_gcode_path: str,
    layer_height: float = 0.2,
    extrusion_width: float = 0.45,
    ...
    Always forces ``--support-material=0`` so that no supports are generated,
    even when a user-supplied config.ini enables them.  The test is designed
    to evaluate overhang quality without support structures.

    When *config_ini* is ``None``, :data:`OVERHANG_SLICER_ARGS` are
    applied together with the explicit *layer_height* and *extrusion_width*.
    ...
    # Always force supports off so the overhang test prints without
    # support structures, even when a user-supplied config.ini enables them.
    cli_extra.append("--support-material=0")
...
3 perimeters and 20% infill provide dimensional accuracy for measuring
tolerance and fit.  5 top and 4 bottom solid layers ensure fully sealed
surfaces.  Same settings as :data:`SHRINKAGE_SLICER_ARGS`.
"""


def slice_tolerance_specimen(
    stl_path: str,
    output_gcode_path: str,
    layer_height: float = 0.2,
    extrusion_width: float = 0.45,
    ...
    When *config_ini* is ``None``, :data:`TOLERANCE_SLICER_ARGS` are
    applied together with the explicit *layer_height* and *extrusion_width*.
...
Same structural settings as the temperature tower — 2 perimeters and 15%
infill.  PrusaSlicer's automatic fan management (cooling) is always forced
off so that G-code ``M106`` commands can control fan speed directly.
"""


def slice_cooling_specimen(
    stl_path: str,
    output_gcode_path: str,
    layer_height: float = 0.2,
    extrusion_width: float = 0.45,
    ...
    Always forces ``--cooling=0`` to disable PrusaSlicer's automatic fan
    management, even when a user-supplied config.ini enables it.  The
    G-code ``M106`` commands inserted during post-processing handle fan
    speed control directly.

    When *config_ini* is ``None``, :data:`COOLING_SLICER_ARGS` are
    applied together with the explicit *layer_height* and *extrusion_width*.
    ...
    # Always disable PrusaSlicer's automatic fan management so that
    # M106 G-code commands can control fan speed directly, even when
    # a user-supplied config.ini enables cooling.
    cli_extra.append("--cooling=0")
