"""Tests for filament_calibrator.tolerance_cli -- tolerance calibration CLI orchestration."""
...
from filament_calibrator.tolerance_cli import (
    build_parser,
    main,
    run,
)
...
class TestBuildParser:
    ...
    def test_defaults(self):
        ...
        assert args.diameters == "3,5,8,10,12,15"
...
class TestRun:
    ...
    @patch("filament_calibrator.tolerance_cli.gl.save")
    @patch("filament_calibrator.tolerance_cli.gl.load")
    @patch("filament_calibrator.tolerance_cli.slice_tolerance_specimen")
    @patch("filament_calibrator.tolerance_cli.generate_tolerance_stl")
    def test_full_pipeline_no_upload(
        ...
        mock_gen.return_value = str(tmp_path / "tolerance.stl")
    ...
    @patch("gcode_lib.compute_bed_center", return_value="125,110")
    @patch("gcode_lib.resolve_printer", return_value="COREONE")
    @patch("filament_calibrator.tolerance_cli.gl.prusalink_upload")
    @patch("filament_calibrator.tolerance_cli.gl.save")
    @patch("filament_calibrator.tolerance_cli.gl.load")
    @patch("filament_calibrator.tolerance_cli.slice_tolerance_specimen")
    @patch("filament_calibrator.tolerance_cli.generate_tolerance_stl")
    def test_upload(
        ...
        mock_slice.return_value = MagicMock(ok=True)
        mock_load.return_value = MagicMock(lines=[])
        mock_upload.return_value = "tolerance.gcode"
    ...
    @patch("filament_calibrator.tolerance_cli.load_config", return_value={})
    ...
    def test_keep_files(
        ...
        stl = tmp_path / "tolerance_test_PLA_abc12.stl"
        raw = tmp_path / "tolerance_test_PLA_abc12_raw.bgcode"
        stl.touch()
        raw.touch()
    ...
    @patch("filament_calibrator.tolerance_cli._find_config_path", return_value=None)
    @patch("gcode_lib.patch_slicer_metadata")
    @patch("gcode_lib.inject_thumbnails")
    @patch("gcode_lib.compute_bed_shape", return_value="0x0,250x0,250x220,0x220")
    @patch("gcode_lib.compute_bed_center", return_value="125,110")
...
class TestMain:
    @patch("filament_calibrator.tolerance_cli.run")
