...
class TestRun:
    ...
    def test_invalid_spans_exits(self, tmp_path):
        """Non-numeric --spans value triggers sys.exit."""
        args = self._make_args(tmp_path, spans="foo")
        with pytest.raises(SystemExit, match="--spans requires comma-separated numbers"):
            run(args)
