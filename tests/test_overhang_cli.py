"""Tests for filament_calibrator.overhang_cli -- overhang calibration CLI orchestration."""
...
from filament_calibrator.overhang_cli import (
    build_parser,
    main,
    run,
)
...
class TestBuildParser:
    ...
    def test_defaults(self):
        ...
        assert args.angles == "20,25,30,35,40,45,50,55,60,65,70"
...
class TestRun:
    ...
    @patch("filament_calibrator.overhang_cli.gl.save")
    @patch("filament_calibrator.overhang_cli.gl.load")
    @patch("filament_calibrator.overhang_cli.slice_overhang_specimen")
    @patch("filament_calibrator.overhang_cli.generate_overhang_stl")
    def test_full_pipeline_no_upload(
        ...
        mock_gen.return_value = str(tmp_path / "overhang.stl")
    ...
    @patch("gcode_lib.compute_bed_center", return_value="125,110")
    @patch("gcode_lib.resolve_printer", return_value="COREONE")
    @patch("filament_calibrator.overhang_cli.gl.prusalink_upload")
    @patch("filament_calibrator.overhang_cli.gl.save")
    @patch("filament_calibrator.overhang_cli.gl.load")
    @patch("filament_calibrator.overhang_cli.slice_overhang_specimen")
    @patch("filament_calibrator.overhang_cli.generate_overhang_stl")
    def test_upload(
        ...
        mock_slice.return_value = MagicMock(ok=True)
        mock_load.return_value = MagicMock(lines=[])
        mock_upload.return_value = "overhang.gcode"
    ...
    @patch("filament_calibrator.overhang_cli.load_config", return_value={})
    ...
    def test_keep_files(
        ...
        stl = tmp_path / "overhang_test_PLA_abc12.stl"
        raw = tmp_path / "overhang_test_PLA_abc12_raw.bgcode"
        stl.touch()
        raw.touch()
    ...
    @patch("filament_calibrator.overhang_cli._find_config_path", return_value=None)
    @patch("gcode_lib.patch_slicer_metadata")
    @patch("gcode_lib.inject_thumbnails")
    @patch("gcode_lib.compute_bed_shape", return_value="0x0,250x0,250x220,0x220")
    @patch("gcode_lib.compute_bed_center", return_value="125,110")
    ...
    def test_invalid_angles_exits(self, tmp_path):
        """Non-integer --angles value triggers sys.exit."""
        args = self._make_args(tmp_path, angles="foo")
        with pytest.raises(SystemExit, match="--angles requires comma-separated integers"):
            run(args)
...
class TestMain:
    @patch("filament_calibrator.overhang_cli.run")
