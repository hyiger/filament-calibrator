"""Tests for filament_calibrator.retraction_speed_cli — retraction speed CLI orchestration."""
...
from filament_calibrator.retraction_speed_cli import (
    MAX_LEVELS,
    _find_config_path,
    _validate_retraction_speed_args,
    build_parser,
    main,
    run,
)
...
class TestBuildParser:
    ...
    def test_defaults(self):
        p = build_parser()
        args = p.parse_args([])
        assert args.retraction_length == 0.8
        assert args.start_speed == 20.0
        assert args.end_speed == 60.0
        assert args.speed_step == 5.0
    ...
    def test_all_options(self):
        ...
        args = p.parse_args([
            ...
            "--output-dir", "/tmp/retraction-speed",
            "--keep-files",
            "-v",
        ])
        assert args.retraction_length == 1.0
        assert args.start_speed == 15.0
        assert args.end_speed == 50.0
        assert args.speed_step == 5.0
...
class TestValidateRetractionSpeedArgs:
    def test_valid_range(self):
        assert _validate_retraction_speed_args(0.8, 20.0, 60.0, 5.0) == 9

    def test_retraction_length_zero_exits(self):
        with pytest.raises(SystemExit, match="positive"):
            _validate_retraction_speed_args(0.0, 20.0, 60.0, 5.0)

    def test_retraction_length_negative_exits(self):
        with pytest.raises(SystemExit, match="positive"):
            _validate_retraction_speed_args(-0.1, 20.0, 60.0, 5.0)

    def test_start_speed_zero_exits(self):
        with pytest.raises(SystemExit, match="positive"):
            _validate_retraction_speed_args(0.8, 0.0, 60.0, 5.0)

    def test_step_zero_exits(self):
        with pytest.raises(SystemExit, match="positive"):
            _validate_retraction_speed_args(0.8, 20.0, 60.0, 0.0)

    def test_step_negative_exits(self):
        with pytest.raises(SystemExit, match="positive"):
            _validate_retraction_speed_args(0.8, 20.0, 60.0, -5.0)

    def test_level_height_zero_exits(self):
        with pytest.raises(SystemExit, match="--level-height must be positive"):
            _validate_retraction_speed_args(0.8, 20.0, 60.0, 5.0, 0.0)

    def test_level_height_negative_exits(self):
        with pytest.raises(SystemExit, match="--level-height must be positive"):
            _validate_retraction_speed_args(0.8, 20.0, 60.0, 5.0, -1.0)

    def test_end_less_equal_start_exits(self):
        with pytest.raises(SystemExit, match="greater than"):
            _validate_retraction_speed_args(0.8, 40.0, 40.0, 5.0)

    def test_end_less_than_start_exits(self):
        with pytest.raises(SystemExit, match="greater than"):
            _validate_retraction_speed_args(0.8, 60.0, 20.0, 5.0)

    def test_not_divisible_exits(self):
        with pytest.raises(SystemExit, match="evenly divisible"):
            _validate_retraction_speed_args(0.8, 20.0, 50.0, 7.0)

    def test_too_many_levels_exits(self):
        with pytest.raises(SystemExit, match="exceeds maximum"):
            _validate_retraction_speed_args(0.8, 1.0, 100.0, 1.0)
...
class TestFindConfigPath:
    ...
    def test_returns_none_when_no_defaults_exist(self):
        ...
        with patch(
            "filament_calibrator.retraction_speed_cli._FIND_CONFIG_PATHS",
...
class TestRun:
    ...
    def _make_args(self, tmp_path, **overrides):
        defaults = dict(
            retraction_length=0.8, start_speed=20.0, end_speed=60.0,
            speed_step=5.0,
    ...
    @patch("filament_calibrator.retraction_speed_cli.gl.save")
    @patch("filament_calibrator.retraction_speed_cli.gl.load")
    @patch("filament_calibrator.retraction_speed_cli.insert_retraction_speed_commands")
    @patch("filament_calibrator.retraction_speed_cli.compute_retraction_speed_levels")
    @patch("filament_calibrator.retraction_speed_cli.slice_retraction_specimen")
    @patch("filament_calibrator.retraction_speed_cli.generate_retraction_tower_stl")
    ...
    @patch("gcode_lib.patch_slicer_metadata")
    @patch("gcode_lib.inject_thumbnails")
    @patch("filament_calibrator.retraction_speed_cli.gl.prusalink_upload")
    @patch("filament_calibrator.retraction_speed_cli.gl.save")
    @patch("filament_calibrator.retraction_speed_cli.gl.load")
    @patch("filament_calibrator.retraction_speed_cli.insert_retraction_speed_commands")
    @patch("filament_calibrator.retraction_speed_cli.compute_retraction_speed_levels")
    ...
    @patch("filament_calibrator.retraction_speed_cli.load_config", return_value={})
    @patch("gcode_lib.patch_slicer_metadata")
    @patch("gcode_lib.inject_thumbnails")
    @patch("filament_calibrator.retraction_speed_cli.gl.save")
    @patch("filament_calibrator.retraction_speed_cli.gl.load")
    ...
    def test_keep_files(
        ...
        stl = tmp_path / "retraction_speed_PLA_20.0_5.0x9_abc12.stl"
        raw_gcode = tmp_path / "retraction_speed_PLA_20.0_5.0x9_abc12_raw.bgcode"
    ...
    @patch("filament_calibrator.retraction_speed_cli.load_config", return_value={})
    @patch("filament_calibrator.retraction_speed_cli._find_config_path",
           return_value=None)
    @patch("gcode_lib.patch_slicer_metadata")
    @patch("gcode_lib.inject_thumbnails")
    @patch("filament_calibrator.retraction_speed_cli.gl.save")
    ...
    def test_verbose_output(
        ...
        mock_levels.return_value = [
            MagicMock(
                z_start=1.0, z_end=2.0, speed_mm_s=20.0,
            ),
        ]
        mock_load.return_value = MagicMock(lines=[])
        mock_insert.return_value = []
        ...
        assert "Retraction speed levels:" in captured.out
    ...
    def test_invalid_printer_exits(
        ...
        with patch(
            "filament_calibrator.retraction_speed_cli.gl.resolve_printer",
    ...
    def test_enclosure_filament_disables_cool_fan(
        ...
        with patch(
            "filament_calibrator.retraction_speed_cli.gl.render_start_gcode",
            return_value="start",
        ) as mock_start, patch(
            "filament_calibrator.retraction_speed_cli.gl.render_end_gcode",
            return_value="end",
        ):
            run(args)
    ...
    @patch("filament_calibrator.retraction_speed_cli.slice_retraction_specimen")
    @patch("filament_calibrator.retraction_speed_cli.generate_retraction_tower_stl")
    def test_prints_speed_table(
        self, mock_gen, mock_slice, mock_levels,
        mock_insert, mock_load, mock_save, mock_inject, mock_patch_meta,
        tmp_path, capsys
    ):
        """Pipeline prints a retraction speed lookup table."""
        mock_gen.return_value = str(tmp_path / "tower.stl")
        mock_slice.return_value = MagicMock(ok=True)
        mock_levels.return_value = [
            MagicMock(z_start=1.0, z_end=2.0, speed_mm_s=20.0),
            MagicMock(z_start=2.0, z_end=3.0, speed_mm_s=25.0),
        ]
        mock_load.return_value = MagicMock(lines=[])
        mock_insert.return_value = []
        ...
        captured = capsys.readouterr()
        assert "Retraction speed by height:" in captured.out
        assert "20.0 mm/s" in captured.out
        assert "25.0 mm/s" in captured.out
...
class TestMain:
    @patch("filament_calibrator.retraction_speed_cli.run")
